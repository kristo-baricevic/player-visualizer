...
const TrackButtons: React.FC<TrackButtonProps> = ({ isMuted, onToggleMute }) => (
    <div className="flex flex-row items-center justify-center z-20 mt-6">
      {[0, 1, 2].map((trackIndex) => (
        ...
        >
          {trackIndex === 0 ? (
            <FontAwesomeIcon icon={faGuitar} size="2xl" color={isMuted[trackIndex] ? "#ccc" : "#000"} />
          ) : trackIndex === 1 ? (
            <FontAwesomeIcon icon={faMicrophone} size="2xl" color={isMuted[trackIndex] ? "#ccc" : "#000"} />
          ) : (
            <FontAwesomeIcon icon={faDrum} size="2xl" color={isMuted[trackIndex] ? "#ccc" : "#000"} />
          )}
        </button>
      ))}
    </div>
